...
#### Acceptance Criteria
- [x] Record, stop (by tap or by itself), replay, verdict and readings work in Chrome with a fake mic; ~~and on the Pixel (with R2-T08)~~ → the Pixel check moves to R3 practice takes (owner's instruction, 2026-09-26)
...
### R2-T08: Owner-voice checks on the Pixel (V1) → moved to R3
**Type:** validation | **Effort:** S | **Depends on:** R2-T07

**Changed (owner's instruction, 2026-09-26):** no separate test session. The owner practises in R3 as normal, then sends one ZIP of practice takes. Claude runs the same checks on those takes (pitch on the sentence frames voiced in both, instead of held vowels). The readings stay labelled "test readings" until then. The task becomes the last task of R3.
